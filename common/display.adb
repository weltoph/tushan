...
package body Display is
  ...
  function Display(Display_Board: Game_Board.Board_T) return Board_Display_T is
    ...
    type Tile_Display is array (1 .. 5, 1 .. 5) of Wide_Character;
    ...
    begin
      ...
      if Game_Board.Is_Occupied(Display_Board, Point) then
        ...
        begin
          ...
          if Northern = Open then
            Tile(3, 2) := '↑';
          end if;

          if Southern = Open then
            Tile(3, 4) := '↓';
          end if;

          if Eastern = Open then
            Tile(4, 3) := '→';
          end if;

          if Western = Open then
            Tile(2, 3) := '←';
          end if;
    ...
    for X in Game_Board.X_Coordinate loop
      for Y in Game_Board.Y_Coordinate loop
        declare
          ...
          Ref_X: constant Natural := (X - 1)*5;
          Ref_Y: constant Natural := (Y - 1)*5;
        begin
          for Xp in 1 .. 5 loop
            for Yp in 1 .. 5 loop
