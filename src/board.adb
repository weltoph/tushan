package body Board is
  ...
  procedure Place_Piece (Board: In Out Board_T;
                         Piece: In Piece_T;
                         At_Point: In Point_T) is
  begin
    for X in Piece'Range(1) loop
      for Y in Piece'Range(2) loop
        declare
          Current_X: constant X_Coordinate_T := At_Point.X
            + (X - Piece'First(1) + 1);
          Current_Y: constant Y_Coordinate_T := At_Point.Y
            + (Y - Piece'First(2) + 1);
        begin
          case Board(Current_X, Current_Y).Status is
            when Occupied =>
              raise Board_Error with "Try to place overlapping piece";
            when Empty => Board(Current_X, Current_Y) := Piece(X, Y);
          end case;
        end;
      end loop;
    end loop;
  ...
  begin
    ...
    return Piece;
  end New_Piece_From_Borders;

  function Get_Status (Board: In Board_T; At_Point: In Point_T)
    return Board_Field_Status_T is
  begin
    return Board(At_Point.X, At_Point.Y).Status;
  end Get_Status;

  function Get_Field (Board: In Board_T; At_Point: In Point_T)
    return Field_T is
  begin
    case Board(At_Point.X, At_Point.Y).Status is
      when Empty => raise Board_Error with "Cannot get empty field.";
      when Occupied => return Board(At_Point.X, At_Point.Y).Field;
    end case;
  end Get_Field;
